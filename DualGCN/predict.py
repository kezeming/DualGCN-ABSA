...
class Instructor:
    ...
    def __init__(self, opt):
        ...
        if 'bert' in opt.model_name:
            ...
            valset = ABSAGCNData(opt.dataset_file['val'], tokenizer, opt=opt)
        ...
        """
        self.val_dataloader = DataLoader(dataset=valset, batch_size=opt.batch_size)
    ...
    def _val(self):
        ...
        with torch.no_grad():
            for batch, sample_batched in enumerate(self.val_dataloader):
                inputs = [sample_batched[col].to(self.opt.device) for col in self.opt.inputs_cols]
                targets = sample_batched['polarity'].to(self.opt.device)
                outputs, penal = self.model(inputs)
                n_test_correct += (torch.argmax(outputs, -1) == targets).sum().item()

                # polarity = polarity_dict_bc[torch.argmax(outputs, -1)]
                print(targets)
    ...
    def run(self):
        ...
        self.model.load_state_dict(torch.load('./DualGCN/state_dict/dualgcnbert_twitter_acc_0.7829_f1_0.7680'))
        self.model.eval()
...
def main():
    ...
    dataset_files = {
        ...
        'twitter': {
            ...
            'val': './DualGCN/dataset/Tweets_corenlp/val.json',
        }
    }
