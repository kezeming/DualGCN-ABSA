...
class ChartParser(nn.Module):
    ...
    def parse_batch(self, sentences, golds=None):
        ...
        if not is_train:
            ...
            for i, (start, end) in enumerate(zip(fp_startpoints, fp_endpoints)):
                ...
                trees.append(tree)
                scores.append(score)
                arc.append(arc_dc_np)

            return trees, scores, arc    # * head是从tree中得到的，tree是从parse_from_annotations中得到的
    ...
    def parse_from_annotations(self, fencepost_annotations_start, fencepost_annotations_end, sentence, sentence_idx, gold=None):
        ...
        if is_train:
            ...
            # print(arc_score.shape)      # torch.Size([9, 9])
            # print(type_score.shape)     # torch.Size([9, 45])  45可能代表 dependency label 的类别？
    ...
    def decode_from_chart(self, sentence, label_scores_chart_np, arc_dc_np, type, sentence_idx=None, gold=None, contributions=None):
        ...
        force_gold = (gold is not None)     # * False
        ...
        # * print("p_father:", p_father)    # [2 4 4 0 4 7 5 4]
        ...
        def make_tree():
            ...
            if (i + 1) >= j:
                ...
                if type is not None:
                    # print("int(i): ", int(i))   # 0
                    # print("tag: ", tag)         # DT 表示限定词
                    # print("word: ", word)       # The
                    # print("p_father[i]: ", p_father[i]) # 2 (head)
                    # print("self.type_vocab.value(type[i]): ", self.type_vocab.value(type[i]))   # det 关系表示
        ...
        tree = tree_list[0]
        return tree, score, arc_dc_np   # * 加了个arc_dc_np
